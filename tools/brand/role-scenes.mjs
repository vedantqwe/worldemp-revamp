/**
 * The drawn fallback scene.
 *
 * Every role is illustrated with a photograph now (see build-role-art.mjs),
 * but the open archives do not have a usable picture of everything, and a
 * page with no art at all is worse than a drawn one. This is what stands in:
 * a flat workspace scene built from the brand palette, the diagram on the
 * wall chosen by discipline and the details varied by a hash of the slug.
 *
 * No text in the artwork - the card and the masthead already name the role,
 * and a drawing that has to be redrawn to be translated goes stale in one
...
export function scene(slug, disciplineKey, { pair = false } = {}) {
  const r = rng(slug);
