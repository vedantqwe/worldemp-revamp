...
There is a CMS at `/admin` — [Sveltia](https://sveltiacms.app), git-based, so
it commits to this repo and the site redeploys itself. No database, no server,
nothing to host.
...
Locally it writes straight to your working copy through the File System Access
API, with **no token and nothing exposed** (Chromium browsers only).

Remotely, at `<site>/admin/`, use **Sign In with Token** — a fine-grained
GitHub token scoped to this repo with `Contents: read and write`. You can save
exactly what your GitHub account is allowed to push, and there's no second
password anywhere.

> **"Sign in with GitHub" needs a small OAuth worker deployed first.** Without
> one it falls back to Netlify's gateway, which only serves Netlify-hosted
> sites, so on GitHub Pages it just says `Not Found`. It's a ten-minute,
> one-time setup — see
> [CLAUDE.md](CLAUDE.md#turning-on-sign-in-with-github).

It edits **overrides**, not sources. `src/content/` is regenerated by the
scraper on every sync, so anything typed into it would vanish at the next
crawl; `content/copy.json` and `content/pages.json` sit on top of it and
survive. A field left blank means *leave the default alone*, so a half-filled
form can't blank the site. See [CLAUDE.md](CLAUDE.md#editing-without-a-developer).

```bash
npm run cms:check    # config parses, overrides point at pages that exist
